...
public class GUIController implements Initializable {
    ...
    /**
     * We use Spring's ApplicationContext container which is more preferable
     * than BeanFactory as it includes all the functionality provided by the
     * BeanFactory container and additionally provides features required for
     ...
     * API which is the most popular configuration mechanism that helps in
     * loading the Beans.xml from the CLASSPATH.
     *
    ...
    /**
     * Client obtains a bean reference using the getBean() method of the
     * ApplicationContext object. The Spring container returns an instance of
     * the bean.
     */
...
//    static ArrayList<User> userList = new ArrayList<>();
    ...
    public void initialisation() {
        ...
        if (initialisationFlag == false) {
            ...
            try {
                ...
                /**
                 * METHOD 1: Using setter-based dependency injection to
                 * initialise objects.
                 *
                 * Setter-based injection occurs when the setter method of the
                 * component is invoked with the dependent component.
                 *
                 * Dependency injection helps to bind independent components
                 * together without using the new operator.
                 */
                member1.setUser("Ajwad Alias", "ajwadalias", "ajwad123", "Member");
                member2.setUser("Ashraf Alias", "ashrafalias", "ashraf123", "Member");
                librarian1.setUser("Muhd Rahiman", "mdrhmn", "ray123", "Librarian");

                user.userList.add(member1);
                user.userList.add(member2);
                user.userList.add(librarian1);

                for (int i = 1; i <= 5; i++) {
                    
                    // Declare a new Book instance
                    Book book = (Book) context.getBean("book"); 
                    
                    // Initialise using setter-based dependency injection
                    book.setBook("Book " + i, "Author " + i, 2000 + i, 3, 0);
                    
                    // Add book into bookStorage books list
                    bookStorage.books.add(book);
                }

                /**
                 * METHOD 2: Using setApplicationContext in order to initialize
                 * the object using dependency injection.
                 *
                 * Spring injects the application context into the parameter of
                 * the setApplicationContext() method in order to initialize the
                ...
                // Set flag to true to ensure one-time invocation
    ...
    /**
     * Controller for Manage Books for Librarian
     */
    ...
    public void memberBorrowBooksController() {
        ...
        if (reserve_book_btn != null) {
            reserve_book_btn.setOnAction((t) -> {
                Boolean reservationStatus = reservation.makeReservation((Member) loggedUser, bookStorage.getBookByID(Integer.parseInt(book_id_field.getText())));
                ...
                if (reservationStatus) {
                    ...
                    Alert alert = new Alert(AlertType.ERROR);
                    alert.setTitle("Error");
                    alert.setHeaderText("You have already reserved " + reservedBookName + " book!");
                    alert.show();
                }
